...
static bool wows_load_hull_info_from_gp(PyObject *mod, PyObject *gp, const char *ship_name, const char *hull_sel,
    ...
    if (!ship_data) {
        ...
        while (PyDict_Next(root, &pos, &pk, &pv)) {
            ...
            if (!PyDict_Check(pv))
                continue;
            if (!PyDict_GetItemString(pv, "ShipUpgradeInfo"))
                continue;
            hits.push_back({k, pv});
        }
        if (hits.size() == 1) {
    ...
    PyObject *info = PyObject_CallMethod(mod, "extract_ship", "sO", found_name.c_str(), ship_data);
    Py_DECREF(root);
    if (!info) {
        PyErr_Print();
        return false;
    ...
    if (hull_sel) {
        ...
        while (PyDict_Next(upgrades, &pos, &pk, &pv)) {
            ...
            for (auto &c : kl)
                c = (char)tolower((unsigned char)c);
            if (kl.find(sel) != std::string::npos) {
                upg_data = pv;
                break;
            }
        }
