                                 ...
                                 uint16_t *out_vtype)
{
    ...
    *out_vtype = 0;
    ...
    for (uint32_t pass = 0; pass <= rank; pass++) {
        ...
        if (pass == rank) { *out_vtype = cur_vt; return cur_off; }
                                   ...
                                   const uint32_t *sections, uint32_t n_sections)
{
    ...
    tinygltf::Mesh mesh;
    for (uint32_t i = 0; i < n_ibloc; i++) {
        if (iinfo[i].count == 0) continue;
        uint16_t k  = iinfo[i].vtype;
        if (k >= n_vtypes || pos_acc[k] < 0) continue;

        int bv_idx  = add_bv(iinfo[i].bv_offset,
                             (size_t)iinfo[i].count * iinfo[i].idx_size,
